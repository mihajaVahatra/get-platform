...
function ReportCard({ icon: Icon, title, description, loading, onDownload }: { icon: typeof FileSpreadsheetIcon; title: string; description: string; loading: boolean; onDownload: () => void }) { return <Card><CardContent className="p-6"><span className="grid size-12 place-items-center rounded-xl bg-violet-50 text-violet-600"><Icon className="size-6" /></span><h2 className="mt-4 font-extrabold text-[#17204e]">{title}</h2><p className="mt-2 min-h-10 text-sm text-slate-500">{description}</p><Button className="mt-5 w-full" onClick={onDownload} disabled={loading}>{loading ? 'Préparation...' : <><DownloadIcon /> Télécharger le CSV</>}</Button></CardContent></Card>; }
