...
/** Formate une date ISO en libellé court « mois année » localisé en français (ex. « janv. 26 »). */
const month = (value: string) => new Intl.DateTimeFormat('fr-FR', { month: 'short', year: '2-digit' }).format(new Date(value));

/**
 * Courbe SVG affichant l'évolution mensuelle du nombre de candidatures sur
 * les 6 derniers mois. Récupère les points via
 * `GET /schools/me/reports/trend?months=6` au montage.
 */
export function TrendLine() {
  ...
  // Dimensions et marges du repère SVG (en unités de viewBox), utilisées pour projeter les points.
  ...
  // Projection des points de données dans le repère SVG : répartition uniforme sur l'axe X,
  // hauteur sur l'axe Y proportionnelle au compteur (0 = bas, max = haut).
